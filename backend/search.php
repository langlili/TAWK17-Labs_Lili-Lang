...
# Here's the query using bound result parameters
    ...
    while ($stmt->fetch()) { //Fetches results from prepared statement - all the data/results go inside while loop. 
        if($onloan == 0) {
            $onloan = 'NO';
            echo "<tr>";
            echo '<td><input type="hidden" name="book_id" value=' . $bookid . '>';
            echo "</td> <td> $title </td><td> $auth_fname $auth_lname </td> <td> $onloan </td>";
            echo '<td><input type="submit" name="deletebook" value="DELETE"></td>';
            echo "</tr>";
            ...
            echo '<td><a href="deleteBook.php?bookid=' . urlencode($bookid) . '"> Delete Book </a></td>';
            echo "</tr>";
        }
    }
    echo "</table>";
